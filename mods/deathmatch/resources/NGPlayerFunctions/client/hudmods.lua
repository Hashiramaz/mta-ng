...
function createBottomMovingText ( text )
	setPlayerHudComponentVisible ( "radio", not customRadio )
	setPlayerHudComponentVisible ( "area_name", not customZones )
	setPlayerHudComponentVisible ( "vehicle_name", not customVehicleNames )
...
addEventHandler ( "onClientUserSettingChange", root, function ( g, v )
	...
	if ( g == "usersettings_usecustomradio" ) then
		customRadio = v
		setPlayerHudComponentVisible ( "radio", not v )
	elseif ( g == "usersettings_usecustomvehiclenames" ) then
		customVehicleNames = v
		setPlayerHudComponentVisible ( "vehicle_name", not v )
	elseif ( g == "usersettings_usecustomhud" ) then
		...
		setPlayerHudComponentVisible ( "area_name", not tobool ( v ) )
	end
end )