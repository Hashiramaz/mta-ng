...
function walk.onHit ( p )
	if ( p and p == localPlayer and p.interior == source.interior and p.dimension == source.dimension ) then
		...
		setTimer ( function ( source )
			...
			tempPed:setPedControlState ( "forwards", true )
		...
		if ( v[2] == walk.runMode ) then 
			if ( walk.wModes[i+1] ) then 
				...
				for i, v in pairs ( walk.wModes ) do 
					tempPed:setPedControlState ( v[1], false )
				end 
				tempPed:setPedControlState ( "forwards", true )
				tempPed:setPedControlState ( walk.wModes[i+1][1], true )
			else
				...
				tempPed:setPedControlState ( "forwards", true )
				tempPed:setPedControlState ( walk.wModes[1][1], true )
			end 
			break
		end 
	end 
	...
	for i, v in ipairs ( walk.wModes ) do
		if ( v[2] == walk.runMode ) then 
			if ( walk.wModes[i-1] ) then 
				...
				tempPed:setPedControlState ( "forwards", true )
				tempPed:setPedControlState ( walk.wModes[#walk.wModes][1], true )
			end 
			break
		end 
	end 
